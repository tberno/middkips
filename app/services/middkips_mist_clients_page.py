...
def render_table(title, rows, max_rows=100):
    ...
    preferred = [
        ...
        "os", "model", "manufacturer", "last_seen", "rx_bytes", "tx_bytes",
        "rssi", "snr"
    ]
    ...
    if len(rows) > max_rows:
        more = f'<p class="muted">Showing {h(max_rows)} of {h(len(rows))} rows.</p>'
...
def render_mist_clients_page(q="", limit=50):
    q = (q or "").strip()
    ...
    form = f'''
    <section class="panel">
      <h1>Mist Client Search</h1>
      <form class="unused-controls" method="get" action="/tools/mist/clients">
        ...
        <a class="button" href="/tools/mist/clients">Clear</a>
        <a class="button" href="/tools/mist/topology?q={h(q)}">Topology</a>
      </form>
      <p class="muted">This page searches Mist client data only. Client data is not mixed into topology by default.</p>
    </section>
    '''
    ...
    if not q:
        ...
        <section class="panel">
          ...
          <p class="muted">Search by MAC, IP, hostname, username, SSID, AP, or site. Results are pulled from the Mist client match data.</p>
        </section>
        '''
